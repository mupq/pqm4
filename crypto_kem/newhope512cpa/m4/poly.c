...
*
* Arguments:   int16_t x: input integer to be reduced
*              
* Returns integer in {0,...,q-1} congruent to x modulo q
**************************************************/
static uint16_t coeff_freeze(int16_t x)
{
  int16_t m,r;
  int16_t c;
  r = x - ((x*5)>>16)*NEWHOPE_Q;
  m = r + NEWHOPE_Q;
  c = r >> 15;
  r = r ^ ((r^m)&c);
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* generate a in blocks of 64 coefficients */
  {
    ...
    while(ctr < 64) /* Very unlikely to run more than once */
    {
      ...
      for(j=0;j<SHAKE128_RATE && ctr < 64;j+=2)
      {
        ...
        if(val < 5*NEWHOPE_Q)
        {
          a->coeffs[i*64+ctr] = (int16_t)(val - ((val*5)>>16)*NEWHOPE_Q);
          ctr++;
        }
      }
    }
...
//  uint32_t t, d, a, b, c;
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* Generate noise in blocks of 64 coefficients */
  {
    ...
    for(j=0;j<64;j++)
    {
      a = buf[2*j];
      b = buf[2*j+1];
      r->coeffs[64*i+j] = (int16_t) hw(a) - hw(b);
      /*
...
void poly_ntt(poly *r)
{
  ...
  ntt((int16_t *)r->coeffs, gammas_bitrev_montgomery);
}
...
void poly_invntt(poly *r)
{
  ...
  ntt((int16_t *)r->coeffs, omegas_inv_bitrev_montgomery);
