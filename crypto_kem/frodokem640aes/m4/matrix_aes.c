...
int mul_add_as_plus_e(uint16_t *out, const uint16_t *s, const uint8_t *seed_A) {
    ...
    aes128leaktimectx ctx128;

    aes128_ecb_keyexp_leaktime(&ctx128, seed_A);
    ...
    for (i = 0; i < PARAMS_N; i += 4) {
        ...
        aes128_ecb_leaktime((uint8_t *)a_row, (uint8_t *)a_row_temp, 4 * PARAMS_N * sizeof(int16_t) / AES_BLOCKBYTES, &ctx128);
...
{ // Generate-and-multiply: generate matrix A (N x N) column-wise, multiply by s' on the left.
  ...
  // Output: out = s'*A + e' (N_BAR x N)
    ...
    aes128leaktimectx roundkeys;
    aes128_ecb_keyexp_leaktime(&roundkeys, seed_A);
    ...
    for (kk = 0; kk < PARAMS_N; kk += PARAMS_STRIPE_STEP) {
        ...
        aes128_ecb_leaktime((unsigned char *)a_cols, (unsigned char *)a_cols_temp, PARAMS_N*PARAMS_STRIPE_STEP*sizeof(int16_t) / AES_BLOCKBYTES, &roundkeys);
