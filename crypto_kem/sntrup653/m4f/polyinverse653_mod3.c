...
#define _p 656 // for jumpdivsteps
#define over_divsteps ((_p-p)*2+1)
#define reverse_head (p-1+over_divsteps)

extern int jump1312divsteps_mod3_64(int minusdelta, uint32_t *M, uint32_t *f, uint32_t *g);
...
int R3_recip_jumpdivsteps(int8_t* H, int8_t* G){
    ...
    minusdelta = jump1312divsteps_mod3_64(minusdelta,(uint32_t*)M,(uint32_t*)f,(uint32_t*)g);
