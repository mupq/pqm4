...
{ // Frodo-KEM's key generation
  ...
  //          secret key sk (CRYPTO_BYTES + BYTES_SEED_A + (PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8 + 2*PARAMS_N*PARAMS_NBAR bytes)
    ...
    uint16_t B[PARAMS_N*PARAMS_NBAR] = {0}, S[PARAMS_N*PARAMS_NBAR] = {0};

    // Generate the secret value s, the seed for S and E, and the seed for A. Add seed_A to the public key
    randombytes(sk, 2*CRYPTO_BYTES + BYTES_SEED_A);
    cshake(pk, BYTES_SEED_A, 0, sk + 2*CRYPTO_BYTES, (unsigned long long)(BYTES_SEED_A));
	
    // Generate S and E, and compute B = A*S + E. Generate A on-the-fly
    frodo_sample_n(S, PARAMS_N*PARAMS_NBAR, sk + CRYPTO_BYTES, CRYPTO_BYTES, 1);
    frodo_sample_n(B, PARAMS_N*PARAMS_NBAR, sk + CRYPTO_BYTES, CRYPTO_BYTES, 2);
    frodo_mul_add_as_plus_e(B, S, pk);
...
{ // Frodo-KEM's key encapsulation
    ...
    uint16_t V[PARAMS_NBAR*PARAMS_NBAR]= {0}, C[PARAMS_NBAR*PARAMS_NBAR] = {0};
    ...
    frodo_sample_n(Bp, PARAMS_N*PARAMS_NBAR, G, CRYPTO_BYTES, 5);
    frodo_mul_add_sa_plus_e(Bp, Sp, temp);
...
{ // Frodo-KEM's key decapsulation
    ...
    frodo_sample_n(Sp, PARAMS_N*PARAMS_NBAR, G, CRYPTO_BYTES, 4);
    frodo_sample_n(BBp, PARAMS_N*PARAMS_NBAR, G, CRYPTO_BYTES, 5);
    frodo_mul_add_sa_plus_e(BBp, Sp, seed_A);

    // Generate Epp, and compute W = Sp*B + Epp
    frodo_sample_n(W, PARAMS_NBAR*PARAMS_NBAR, G, CRYPTO_BYTES, 6);
    frodo_unpack(B, PARAMS_N*PARAMS_NBAR, temp+BYTES_SEED_A, CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A, PARAMS_LOGQ);
    frodo_mul_add_sb_plus_e(W, B, Sp);
