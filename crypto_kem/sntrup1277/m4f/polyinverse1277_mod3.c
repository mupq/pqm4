...
#define _p 1280 // for jumpdivsteps
#define over_divsteps ((_p-p)*2+1)
#define reverse_head (p-1+over_divsteps)

extern int jump2560divsteps_mod3_128(int minusdelta, uint32_t* M, uint32_t* f, uint32_t* g);
...
int R3_recip_jumpdivsteps(int8_t* H, int8_t* G){
    ...
    minusdelta = jump2560divsteps_mod3_128(minusdelta,(uint32_t*)M,(uint32_t*)f,(uint32_t*)g);
