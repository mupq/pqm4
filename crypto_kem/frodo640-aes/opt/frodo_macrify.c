...
{ // Generate-and-multiply: generate matrix A (N x N) column-wise, multiply by s' on the left.
  ...
  int i, j, kk, k;
    ...
    uint16_t a_cols[PARAMS_N*PARAMS_STRIPE_STEP];
    uint16_t a_cols_temp[PARAMS_N*PARAMS_STRIPE_STEP];       
        ...
        for (i = 0; i < PARAMS_NBAR; i++) {
            for (k = 0; k < PARAMS_STRIPE_STEP; k += PARAMS_PARALLEL) {
                ...
                for (j = 0; j < PARAMS_N; j++) {                // Matrix-vector multiplication
                    ...
                    sum[0] += sp * a_cols[j*PARAMS_STRIPE_STEP + (k+0)];
                    sum[1] += sp * a_cols[j*PARAMS_STRIPE_STEP + (k+1)];
                    sum[2] += sp * a_cols[j*PARAMS_STRIPE_STEP + (k+2)];
                    sum[3] += sp * a_cols[j*PARAMS_STRIPE_STEP + (k+3)];
                }
