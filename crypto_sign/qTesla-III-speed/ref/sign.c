...
{ // Encode signature sm
  ...
  for (i=0; i<(PARAM_N*PARAM_D/8); i+=(PARAM_D/2)) {
    pt[i   ] = (t[j] & ((1<<8)-1));
    pt[i+ 1] = (t[j] >> 8) & ((1<<8)-1); 
    pt[i+ 2] = (t[j+1]<<6) | ((t[j]>>16) & ((1<<6)-1));
    pt[i+ 3] = (t[j+1]>>2) & ((1<<8)-1);
    pt[i+ 4] = (t[j+1]>>10) & ((1<<8)-1);
    pt[i+ 5] = (t[j+2]<<4) | ((t[j+1]>>18) & ((1<<4)-1));
    pt[i+ 6] = (t[j+2]>>4) & ((1<<8)-1);
    pt[i+ 7] = (t[j+2]>>12) & ((1<<8)-1);
    pt[i+ 8] = (t[j+3]<<2) | ((t[j+2]>>20) & ((1<<2)-1));
    pt[i+ 9] = (t[j+3]>>6) & ((1<<8)-1);
    pt[i+10] = (t[j+3]>>14) & ((1<<8)-1);
    j += 4;
  }
...
{ // Decode signature sm
  ...
  for (i=0; i<PARAM_N; i+=4) {
    z[i   ] = ((int32_t)(((uint32_t)pt[j]) | ((uint32_t)pt[j+1]<<8) | ((uint32_t)pt[j+2]<<16))<<10)>>10;
    z[i+ 1] = ((int32_t)(((uint32_t)pt[j+2]>>6) | (((uint32_t)pt[j+3])<<2) | (((uint32_t) pt[j+4])<<10) | ((uint32_t)pt[j+5]<<18))<<10)>>10;
    z[i+ 2] = ((int32_t)(((uint32_t)pt[j+5]>>4) | (((uint32_t)pt[j+6])<<4) | ((uint32_t) pt[j+7]<<12) | ((uint32_t) pt[j+8]<<20))<<10)>>10;
    z[i+ 3] = ((int32_t)(((uint32_t)pt[j+8]>>2) | (((uint32_t)pt[j+9])<<6) | (((uint32_t)pt[j+10])<<14))<<10)>>10;
    j += 11;
  }   
