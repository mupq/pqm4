...
void polymatkl_expand_frozen(polyvecl_frozen mat[K], const uint8_t rho[SEEDBYTES]) {
    ...
    for (i = 0; i < K; ++i)
        for (j = 0; j < M; ++j)
            poly_uniform_frozen(&mat[i].vec[j + 1], rho, (i << 8) + j);
}
...
void polymatkl_double_frozen(polyvecl_frozen mat[K]) {
    ...
    for (i = 0; i < K; ++i) {
        for (j = 1; j < L; ++j) {
            for (k = 0; k < N; ++k) {
                mat[i].vec[j].coeffs[k] = (uint16_t) freeze(2 * ((int32_t) mat[i].vec[j].coeffs[k]));
            }
        }
    }
}
...
void polymatkl_pointwise_montgomery_frozen(polyveck *t, const polyvecl_frozen mat[K],
    ...
    for (i = 0; i < K; ++i) {
        polyvecl_pointwise_acc_montgomery_frozen(&t->vec[i], &mat[i], v);
    }
}
