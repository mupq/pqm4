...
// Polynomial with values in [0,q-1] for use in matrices.
// These are typically read-only after initial computation.
typedef struct {
    uint16_t coeffs[N];
} poly_frozen;
...
#define poly_frommont HAETAE_NAMESPACE(poly_frommont)
void poly_frommont(poly *v);
#define poly_double_negate HAETAE_NAMESPACE(poly_double_negate)
void poly_double_negate(poly *v);
#define poly_caddq HAETAE_NAMESPACE(poly_caddq)
void poly_caddq(poly *v);



#define poly_clone HAETAE_NAMESPACE(poly_clone)
...
#define poly_cneg HAETAE_NAMESPACE(poly_cneg)
void poly_cneg(poly *c, const uint8_t b);
#define poly_shl HAETAE_NAMESPACE(poly_shl)
void poly_shl(poly *r, const uint8_t shmnt);
#define poly_double HAETAE_NAMESPACE(poly_double)
...
#define poly_caddDQ2ALPHA HAETAE_NAMESPACE(poly_caddDQ2ALPHA)
void poly_caddDQ2ALPHA(poly *h);
#define poly_highbits_hint HAETAE_NAMESPACE(poly_highbits_hint)
